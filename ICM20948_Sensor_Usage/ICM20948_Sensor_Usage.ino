...
//Connect the SCL pin to the respective SCL port on the Arduino (for the Arduino Uno this is pin A5, Mega 2560 --> A21)
//Connect the SDA pin to the respective SDA port on the Arduino (for the Arduino Uno this is pin A4, Mega 2560 --> A20)
...
ICM_20948_AGMT_t agmt; // Create and agmt objecart for accel, gyro, magn, and temp data

//Booleans for activating or deactivating Digital Low Pass Filters for Accelerometer and Gyroscope
bool enaccDLPF = false;
...
//Set to 1 in order to Enable calibration
    ...
    SERIAL_PORT.println(F("Calibration Finished"));
    Serial.print("\r\n");
#endif
...
// It is then converted from radians to degrees
    ...
    Serial.println(F("Initial Kalman Filter Noise Data Below"));
    Serial.print(KFNoises[0]);
    Serial.print("\t");
    Serial.print(KFNoises[1]);
    Serial.print("\t");
    Serial.println(KFNoises[2]);
    Serial.print("\r\n");
    ...
    Serial.println(F("Set Kalman Filter Noise Data Below"));
    Serial.print(KFNoises[0]);
    Serial.print("\t");
    Serial.print(KFNoises[1]);
    Serial.print("\t");
    ...
    Serial.println(F("Data Collection: "));
    Serial.print("\r\n");
...
//Take norm of magnetometer (not sure why we need norms here)
    ...
    //Printing out Kalman filter data (roll, pitch, and then yaw)
...
//Printing any non Kalman Filtered data (set to 0 to only display Kalman filtered roll, pitch and yaw)
#if 1

//Printing Data Values with Offsets (change to 0 to use metric units rather than MU)
#if 1
...
//Set to 1 in order to enable offsets in data collection
    ...
    SERIAL_PORT.println(F("Calibration offsets shown below"));
